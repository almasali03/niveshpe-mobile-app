...
// ==================== SIPs Tab V2 Functions ====================

// Format date helper
function formatSIPDate(dateString) {
...
function renderSIPSummary() {
    if (typeof sipSummaryV2 === 'undefined') {
        console.log('SIP summary data not available');
        return;
    }

    document.getElementById('sipCountBadge').textContent = `${sipSummaryV2.totalActiveSips} SIP${sipSummaryV2.totalActiveSips !== 1 ? 's' : ''}`;
    document.getElementById('sipTotalInvested').textContent = `₹${formatIndianNumber(sipSummaryV2.totalInvested)}`;
    document.getElementById('sipCurrentValue').textContent = `₹${formatIndianNumber(sipSummaryV2.totalCurrentValue)}`;

    const returnsText = `+₹${formatIndianNumber(sipSummaryV2.totalGain)} (${sipSummaryV2.totalGainPercent}%)`;
    document.getElementById('sipTotalReturns').textContent = returnsText;
    document.getElementById('sipAvgXIRR').textContent = `${sipSummaryV2.averageXIRR}%`;
}

// Render individual SIP card
function createSIPCard(sip) {
    const hasProgress = sip.installments.total !== null;
    const progressPercent = hasProgress ? (sip.installments.completed / sip.installments.total * 100).toFixed(0) : 0;

    // Format next debit date
    const nextDebitFormatted = formatSIPDate(sip.nextDebitDate);

    // Status badge class
    const statusClass = sip.status;
    const statusText = sip.status.charAt(0).toUpperCase() + sip.status.slice(1);

    // Create fund list HTML
    let fundsHTML = '';
    if (sip.funds.length === 1) {
        fundsHTML = `<div class="sip-fund-item">
            <div class="sip-fund-name">${sip.funds[0].name}</div>
        </div>`;
    } else {
        fundsHTML = sip.funds.map(fund => `
            <div class="sip-fund-item">
                <div class="sip-fund-name">${fund.name}</div>
                <div class="sip-fund-amount">₹${formatIndianNumber(fund.amount)}</div>
                <div class="sip-fund-allocation">${fund.allocation}%</div>
            </div>
        `).join('');
    }

    // Performance values
    const returnsClass = sip.performance.absoluteGain >= 0 ? 'positive' : 'negative';
    const returnsSign = sip.performance.absoluteGain >= 0 ? '+' : '';

    return `
        <div class="sip-card" data-sip-id="${sip.sipId}" data-frequency="${sip.frequency}" data-status="${sip.status}">
            <div class="sip-card-header">
                <div class="sip-card-title">
                    <div class="sip-amount">₹${formatIndianNumber(sip.amount)}</div>
                    <span class="sip-status-badge ${statusClass}">${statusText}</span>
                </div>
                <button class="sip-menu-btn" onclick="showSIPMenu(event, '${sip.sipId}')">
                    <svg viewBox="0 0 24 24" fill="currentColor">
            ...
            <div class="sip-frequency">
                <svg class="sip-frequency-icon" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M9 11H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2zm2-7h-1V2h-2v2H8V2H6v2H5c-1.11 0-1.99.9-1.99 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V9h14v11z"/>
                </svg>
                ${sip.frequencyLabel}
            </div>

            <div class="sip-performance-metrics">
                <div class="sip-performance-item">
                    <div class="sip-performance-label">Invested</div>
                    <div class="sip-performance-value">₹${formatIndianNumber(sip.performance.totalInvested)}</div>
                </div>
                <div class="sip-performance-item">
                    <div class="sip-performance-label">Current</div>
                    <div class="sip-performance-value">₹${formatIndianNumber(sip.performance.currentValue)}</div>
                </div>
                <div class="sip-performance-item">
                    <div class="sip-performance-label">Returns</div>
                    <div class="sip-performance-value ${returnsClass}">
                        ${returnsSign}${sip.performance.percentageGain}%
                    </div>
                </div>
            </div>

            ${hasProgress ? `
            <div class="sip-installment-progress">
                <div class="sip-installment-header">
                    <span class="sip-installment-label">Progress</span>
                    <span class="sip-installment-count">${sip.installments.completed}/${sip.installments.total} completed</span>
                </div>
                <div class="sip-installment-bar">
                    <div class="sip-installment-fill" style="width: ${progressPercent}%"></div>
                </div>
            </div>
            ` : ''}

            ${sip.status === 'active' ? `
            <div class="sip-next-debit">
                <svg viewBox="0 0 24 24" fill="currentColor">
                    <path d="M9 11H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2zm2-7h-1V2h-2v2H8V2H6v2H5c-1.11 0-1.99.9-1.99 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V9h14v11z"/>
                </svg>
                <span class="sip-next-debit-text">Next debit on ${nextDebitFormatted}</span>
            </div>
            ` : sip.status === 'paused' && sip.pausedUntil ? `
            <div class="sip-next-debit" style="background: rgba(245, 124, 0, 0.05);">
                <svg viewBox="0 0 24 24" fill="currentColor" style="color: #F57C00;">
                    <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>
                </svg>
                <span class="sip-next-debit-text">Paused until ${formatSIPDate(sip.pausedUntil)}</span>
            </div>
            ` : ''}

            <div class="sip-funds-list">
                ${fundsHTML}
            </div>
        </div>
    `;
...
// Render all SIP cards
function renderSIPCards(sipsToRender = null) {
    const sipList = document.getElementById('sipList');
    const emptyState = document.getElementById('sip-empty-state');

    if (!sipList) return;

    const sipsArray = sipsToRender || (typeof sipsDataV2 !== 'undefined' ? Object.values(sipsDataV2) : []);
    ...
    sipList.innerHTML = sipsArray.map(sip => createSIPCard(sip)).join('');
}

// Sort SIPs
let currentSIPSort = 'amount-high';
function sortSIPs(sortBy) {
    if (typeof sipsDataV2 === 'undefined') return;

    currentSIPSort = sortBy;
    let sipsArray = Object.values(sipsDataV2);

    switch(sortBy) {
        case 'amount-high':
            sipsArray.sort((a, b) => b.amount - a.amount);
            break;
        case 'amount-low':
            sipsArray.sort((a, b) => a.amount - b.amount);
            break;
        case 'returns':
            sipsArray.sort((a, b) => b.performance.percentageGain - a.performance.percentageGain);
            break;
        case 'next-debit':
            sipsArray.sort((a, b) => new Date(a.nextDebitDate) - new Date(b.nextDebitDate));
            break;
        case 'start-date':
            sipsArray.sort((a, b) => new Date(b.startDate) - new Date(a.startDate));
            break;
    }

    renderSIPCards(sipsArray);
}

// Filter SIPs
let currentSIPFilter = 'all';
function filterSIPs(filterBy) {
    if (typeof sipsDataV2 === 'undefined') return;

    currentSIPFilter = filterBy;
    let sipsArray = Object.values(sipsDataV2);

    // Apply filter
    if (filterBy !== 'all') {
        if (filterBy === 'active' || filterBy === 'paused') {
            sipsArray = sipsArray.filter(sip => sip.status === filterBy);
        } else if (filterBy === 'monthly' || filterBy === 'weekly') {
            sipsArray = sipsArray.filter(sip => sip.frequency === filterBy);
        }
    }

    // Re-apply current sort
    sortSIPs(currentSIPSort);
}

// Show SIP menu (placeholder)
function showSIPMenu(event, sipId) {
    event.stopPropagation();
    console.log('Show menu for SIP:', sipId);
    // TODO: Implement three-dot menu with actions
    alert(`SIP Menu for ${sipId}\n\nActions:\n- View Details\n- Modify Amount\n- Skip Next\n- Pause\n- Stop\n- Download Statement`);
}

// Initialize SIPs V2
function initializeSIPsV2() {
    console.log('Initializing SIPs V2...');

    // Check if data is available
    if (typeof sipsDataV2 === 'undefined' || typeof sipSummaryV2 === 'undefined') {
        console.warn('SIP data not available. Make sure funds-data.js is loaded.');
        return;
    }
    ...
    console.log('SIPs V2 initialized successfully');
}
